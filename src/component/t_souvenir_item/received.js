...
class approvalRequest extends Component {
    ...
    async receivedHandler() {
        let token = localStorage.getItem(appconfig.secure_key.token);
        
        let result = await souveniritemapi.insertReceived(this.state.formdataSouvenir);
        if(result.status === 200) {
            console.log("berhasil input data received...");
            document.getElementById("hidePopUpBtnReceived").click();
            this.props.modalStatus(3, "Data Updated! Transaction Souvenir Request with code " + this.props.souvenirRequest.code + " has been received by requester!", this.props.souvenirRequest.code);
        } else {
        ...
        return(
            <div className="modal-content">
                <div className="modal-header">
                    <button id="hidePopUpBtnReceived" type="button" onClick={ this.resetForm } className="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                    <h4 className="modal-title">Received Souvenir Request - {this.state.formdataSouvenir.code}</h4>
                </div>
                ...
                <div className="modal-footer">
                    <button type="button" onClick={ this.receivedHandler } className="btn btn-primary">Received</button>
